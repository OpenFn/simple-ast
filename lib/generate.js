...
function format(exp) {
  ...
  const formattedVariables = exp.exportedVariables.map(function (item, index) {
    ...
    let params = {};
    if (item.declarations[0].init.right) {
      params = item.declarations[0].init.right.arguments[0].params.map(
        (item) => {
          return item.name;
        }
      );
...
if (!filename.includes('language-common')) {
...
fs.writeFile(outputPath, writeable, (err) => {
  ...
  if (errors.length > 0) {
    ...
    console.log('\x1b[33m%s\x1b[0m', "Please make sure you know what you're doing if you publish this as is. 🤔");
  } else {
